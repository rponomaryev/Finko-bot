...
# Anti-spam / cost protection. These limits protect OpenAI tokens from being
# spent by one noisy chat or by a burst of many chats. For multi-worker or
# multi-server deployments, move this state to Redis or another shared store.
...
RATE_LIMIT_NOTICE_COOLDOWN_SECONDS = int(os.getenv("RATE_LIMIT_NOTICE_COOLDOWN_SECONDS", "60"))
OPENAI_COOLDOWN_SECONDS = float(os.getenv("OPENAI_COOLDOWN_SECONDS", "3"))
OPENAI_GLOBAL_WINDOW_SECONDS = int(os.getenv("OPENAI_GLOBAL_WINDOW_SECONDS", "60"))
OPENAI_GLOBAL_MAX_CALLS = int(os.getenv("OPENAI_GLOBAL_MAX_CALLS", "60"))
...
# Rate-limit state. This is intentionally in memory for a simple single-process bot.
...
def is_chat_rate_limited(chat_id: int) -> bool:
...
def can_send_rate_limit_notice(chat_id: int) -> bool:
    now = time.monotonic()
    with rate_limit_lock:
...
def reserve_openai_call(chat_id: int) -> tuple[bool, str | None]:
...
async def telegram_webhook(
    ...
    openai_allowed, openai_limit_reason = reserve_openai_call(chat_id)
    if not openai_allowed:
        ...
        log_event(chat_id, "openai_rate_limited", openai_limit_reason)
